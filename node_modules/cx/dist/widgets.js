...
var Button =
	...
	(function(_HtmlElement) {
		...
		_proto.prepareData = function prepareData(context, instance) {
			var data = instance.data;
			data.stateMods = _objectSpread2({}, data.stateMods, {
	...
	(function(_Container) {
		...
		_proto.add = function add(item) {
			if (item && item.putInto == "header")
				this.header = _objectSpread2({}, item, {
					putInto: null
				});
			else if (item && item.putInto == "footer")
				this.footer = _objectSpread2({}, item, {
					putInto: null
				});
...
var Resizer =
	...
	(function(_Widget) {
		...
		_proto.render = function render(context, instance, key) {
			var data = instance.data;
			return VDOM.createElement(ResizerCmp, {
				key: key,
				instance: instance,
				data: data
			});
...
function initiateDragDrop(e, options, onDragEnd) {
	...
	var source = _objectSpread2({}, options.source, {
...
var OverlayComponent =
	...
	(function(_VDOM$Component2) {
		...
		_proto3.getOverlayStyle = function getOverlayStyle() {
			var data = this.props.instance.data;
			return _objectSpread2({}, data.style, {}, this.customStyle);
		};
	...
	(function(_HtmlElement) {
		...
		_proto.render = function render(context, instance, key) {
			return VDOM.createElement(
				...
				instance.data.text
					? VDOM.createElement("span", null, instance.data.text)
					: this.renderChildren(context, instance)
			);
		};
...
var MenuItemComponent =
	...
	(function(_VDOM$Component) {
		...
		_proto2.render = function render() {
			...
			var arrow =
				data.arrow &&
				VDOM.createElement(DropdownIcon, {
					...
					return VDOM.createElement(
						MenuItemComponent$1,
						{
							...
							hidden:
								...
								(i >= _this2.state.nonOverflownItemCount && i + 1 != children.length),
							instance: instance,
	...
	(function(_VDOM$Component) {
		...
		_proto2.onChange = function onChange(e, change) {
			...
			if (widget.reactOn.indexOf(change) != -1) {
				...
				var value = text || null; //it's important not to set the old value as it causes weird behavior if debounce is used
	...
	(function(_VDOM$Component) {
		...
		_proto2.render = function render() {
			...
			return VDOM.createElement(
				...
				check == "check" &&
					VDOM.createElement(CheckIcon, {
						className: CSS.element(baseClass, "input-check")
					}),
				check == "indeterminate" &&
					VDOM.createElement(SquareIcon, {
						className: CSS.element(baseClass, "input-check")
					})
			);
		};
					...
					VDOM.createElement(
						...
						VDOM.createElement(
							...
							VDOM.createElement(
								...
								VDOM.createElement(ForwardIcon, {
...
var LookupComponent =
			...
			switch (e.keyCode) {
				...
				case KeyCode.enter:
					if (this.props.instance.widget.submitOnEnterKey) {
						var _instance = this.props.instance.parent;

						while (_instance) {
							if (_instance.events && _instance.events.onSubmit) {
								_instance.events.onSubmit(e, _instance);
     ...
     */
			var instance = this.props.instance;
			var widget = instance.widget,
				data = instance.data; //do not make duplicate queries if fetchAll is enabled
			...
			if (widget.onQuery) {
				...
				this.queryTimeoutId = setTimeout(function() {
					...
					var query = (_this12.lastQueryId = Date.now());
					Promise.resolve(result)
						.then(function(results) {
							//discard results which do not belong to the last query
							if (query !== _this12.lastQueryId) return;
...
var ColorPicker =
	...
	(function(_Field) {
		...
		_proto.renderInput = function renderInput(context, instance, key) {
			return VDOM.createElement(ColorPickerComponent, {
				key: key,
				instance: instance
			});
		};
	...
	return {
		...
		onResolve: function onResolve(_ref2) {
			...
			for (var m = 0; m < max; m += step) {
				var time = today + m * 60 * 1000;
				times.push({
					id: time,
					text: Format$1.value(time, format || "datetime;HHmm")
				});
			}
				...
				children: [
					"\n                        ",
					{
						...
						onItemClick: function onItemClick(e, instance) {
							var store = instance.store;
							var value = store.get("$time.id");
							var selection = store.get("$selection");
							var copy = selection ? new Date(selection) : new Date();
						...
						selection: {
							...
							getIsSelectedDelegate: function getIsSelectedDelegate(store) {
								var selection = store.get("$selection");
								if (!selection)
									return function() {
										return false;
									};
								var selectionTime = new Date(selection).valueOf();
								return function(record) {
									return record.id % 86400000 == selectionTime % 86400000;
								};
							}
						},
						jsxSpread: [props],
						jsxAttributes: ["records", "recordAlias", "onItemClick", "selection"],
						children: [
							"\n                           ",
							{
								$type: HtmlElement$1,
	...
	(function(_Field) {
		...
		_proto.prepareData = function prepareData(context, instance) {
			...
			if (data.value) {
				...
				else data.formatted = Format$1.value(date, data.format);
			} else data.formatted = "";
		...
		_proto2.getDropdown = function getDropdown() {
			...
			var dropdown = _objectSpread2(
				...
				widget.dropdownOptions,
				{
					...
					items: _objectSpread2({}, pickerConfig, {}, this.props.picker, {
						autoFocus: true,
						onKeyDown: function onKeyDown(e) {
							return _this2.onKeyDown(e);
						},
	...
	(function(_Widget) {
			...
			this.header.items.forEach(function(line) {
				line.items.forEach(function(c) {
					...
					if (
						...
						_this.hasResizableColumns = true;
		...
		_proto.prepareData = function prepareData(context, instance) {
			...
			data.stateMods =
				...
				(_data$stateMods.resizable = this.hasResizableColumns),
				_data$stateMods);
		...
		_proto.exploreCleanup = function exploreCleanup(context, instance) {
			...
			instance.components.header.children.forEach(function(line) {
						...
						if (header) {
							...
							if (hdwidget.sortable && header.widget.allowSorting) {
								...
								if (data.sorters && data.sorters[0].field == (hdwidget.sortField || hdwidget.field)) {
									...
									sortIcon = VDOM.createElement(DropDownIcon, {
							...
							if (hdwidget.resizable || header.data.resizable) {
								resizer = VDOM.createElement("div", {
		...
		_proto.onHeaderClick = function onHeaderClick(e, column, instance, headerLine) {
			...
			var sortValue = column.sortValue || column.value;
		...
		_proto.renderFixedFooter = function renderFixedFooter(context, instance) {
			...
			var record = records[records.length - 1]; //all type of records are allowed here because the footer can be based on pre-computed data
	...
	(function(_VDOM$Component) {
		...
		_proto2.render = function render() {
			...
			var _this$props = this.props,
				...
				fixedColumnsFixedFooter = _this$props.fixedColumnsFixedFooter;
			var widget = instance.widget,
				hasFixedColumns = instance.hasFixedColumns,
			...
			var cellWrap = function cellWrap(children) {
				return children;
			};
			...
			var addRow = function addRow(record, index, standalone) {
				...
				var wrap = function wrap(children, fixedColumns) {
					return VDOM.createElement(
						GridRowComponent,
						{
							key: key,
							...
							return VDOM.createElement(
								...
								content.map(function(_ref3, cellIndex) {
									var key = _ref3.key,
										...
										uniqueColumnId = _ref3.uniqueColumnId;
									if (Boolean(data.fixed) !== fixedColumns) return null;
									var cellected = index == cursor && cellIndex == cursorCellIndex && widget.cellEditable;
				...
				fixedColumnsContent.push(
					VDOM.createElement(
						"div",
						{
							key: "fcfh",
							ref: function ref(el) {
								_this5.dom.fixedColumnsFixedHeader = el;
							},
						...
						VDOM.createElement("table", null, this.props.fixedColumnsFixedHeader)
					)
				);
			...
			if (fixedFooter || fixedColumnsFixedFooter) {
				content.push(
					VDOM.createElement(
						...
						VDOM.createElement("table", null, fixedFooter)
					)
				);
				if (hasFixedColumns)
					fixedColumnsContent.push(
						VDOM.createElement(
							"div",
							{
								key: "fcff",
								ref: function ref(el) {
									_this5.dom.fixedColumnsFixedFooter = el;
								},
							...
							VDOM.createElement("table", null, fixedColumnsFixedFooter)
						)
					);
			}
			...
			return VDOM.createElement(
				...
				{
					className: data.classNames,
					style: _objectSpread2({}, data.style, {
		...
		_proto2.componentWillReceiveProps = function componentWillReceiveProps(props) {
			var _props$instance = props.instance,
				data = _props$instance.data,
				widget = _props$instance.widget,
				records = _props$instance.records;
			this.setState({
				cursor: Math.max(
				...
				if (this.dom.fixedFooter || this.dom.fixedColumnsFixedFooter) {
					...
					if (this.dom.fixedFooter) {
						...
						if (_dstTableBody) {
							...
							copyCellSize(_srcTableBody, _dstTableBody);
		...
		_proto2.moveCursor = function moveCursor(index, _temp2) {
			...
			if (Object.keys(newState).length > 0) {
				...
				this.setState(newState, function() {
					if (!_this9.state.cellEdit && wasCellEditing) {
						if (_this9.state.focused) FocusManager.focus(_this9.dom.el);

						var record = _this9.getRecordAt(prevState.cursor);

						if ((!_this9.cellEditorValid || cancelEdit) && _this9.cellEditUndoData)
							record.store.set(widget.recordName, _this9.cellEditUndoData);
						else {
							if (widget.onCellEdited && record.data != _this9.cellEditUndoData)
								_this9.props.instance.invoke(
									"onCellEdited",
									{
										column: widget.row.line1.columns[prevState.cursorCellIndex],
										newData: record.data,
										oldData: _this9.cellEditUndoData,
					...
					if (_this9.state.cellEdit && !wasCellEditing)
						_this9.cellEditUndoData = _this9.getRecordAt(_this9.state.cursor).data;

					if (scrollIntoView) {
						var _record = _this9.getRecordAt(index);

						var item = _record && _this9.dom.table.querySelector('tbody[data-record-key="' + _record.key + '"]');

						var hscroll = false;

						if (item) {
							if (widget.cellEditable)
								if (_this9.state.cursorCellIndex >= _this9.props.instance.fixedColumnCount) {
									hscroll = true;
									item =
										item.firstChild.children[_this9.state.cursorCellIndex - _this9.props.instance.fixedColumnCount];
								} else {
									var fixedItem = _this9.dom.fixedTable.querySelector('tbody[data-record-key="' + _record.key + '"]');

									var cell = fixedItem && fixedItem.firstChild.children[_this9.state.cursorCellIndex];
			...
			switch (e.keyCode) {
				...
				case KeyCode.tab:
					if (widget.cellEditable) {
						...
						var cursor = this.state.cursor;
						var cellIndex = (this.state.cursorCellIndex + direction) % widget.row.line1.columns.length;
...
var Pagination =
	...
	(function(_Widget) {
		...
		_proto.render = function render(context, instance, key) {
			...
			var nextPageIcon = VDOM.createElement(ForwardIcon, {
				className: CSS.element(baseClass, "icon-next-page")
			});
			var prevPageIcon = VDOM.createElement(ForwardIcon, {
...
var DragSourceComponent =
	...
	(function(_VDOM$Component) {
		...
		_proto2.render = function render() {
			...
			var eventHandlers = _objectSpread2({}, instance.getJsxEventProps(), {
				onTouchStart: this.onMouseDown,
				onMouseDown: this.onMouseDown,
	...
	(function(_VDOM$Component) {
		...
		_proto2.render = function render() {
			...
			return VDOM.createElement(
				...
				{
					className: CSS.expand(classes),
					style: _objectSpread2({}, data.style, {}, this.state.style, {}, stateStyle),
					ref: function ref(el) {
						_this2.el = el;
					}
				},
