...
Controller.factory = function(alias, config, more) {
	if (isFunction(alias)) {
		var cfg = _objectSpread2({}, config, {}, more);
		...
		return Controller.create(_objectSpread2({}, config, {}, more, {}, result));
	}

	return Controller.create(_objectSpread2({}, config, {}, more));
};
...
var Localization =
	...
	(function() {
		...
		Localization.localize = function localize(culture, key, values) {
			...
			l[key] = _objectSpread2({}, l[key], {}, values);
		};
...
var ArrayAdapter =
	...
	(function(_DataAdapter) {
		...
		_proto.buildSorter = function buildSorter(sorters) {
			...
			var compare = this.sortOptions ? Culture.getComparer(this.sortOptions) : null;
...
var Instance =
	...
	(function() {
		...
		_proto.set = function set(prop, value, internal) {
			var _this3 = this;
		...
		_proto.definePropertySetter = function definePropertySetter(prop, setter, internal) {
...
var DetachedScope =
	...
	(function(_IsolatedScope) {
		...
		_proto.init = function init() {
			...
			delete this.children;
			if (this.name)
				this.options = _objectSpread2({}, this.options, {
					name: this.name
				});
				...
				onSet: function onSet(path, value) {
					...
					if (!config || (!config.bind && !config.set))
						throw new Error(
							"Cannot change " +
								path +
								" in Restate as it's read-only. It's not a binding and it doesn't have a set function either."
						);
					if (config.bind)
...
var PrivateStore =
	/*#__PURE__*/
	(function(_Restate) {
		_inheritsLoose(PrivateStore, _Restate);

		function PrivateStore() {
			return _Restate.apply(this, arguments) || this;
		}
...
var DataProxyView =
	...
	(function(_ReadOnlyDataView) {
		...
		_proto2.getAdditionalData = function getAdditionalData(parentStoreData) {
			if (!this.cached || this.meta.version !== this.cache.version) this.data = this.dataSelector(parentStoreData);
			return this.data;
		};
...
var flattenProps = function flattenProps(props) {
	...
	if (props.$props !== undefined) {
		props = _objectSpread2({}, props.$props, {
	...
	(function(_ArrayAdapter) {
		...
		_proto.processLevel = function processLevel(keys, records, result, parentStore) {
			...
			results.forEach(function(gr) {
				...
				var $group = _objectSpread2({}, gr.key, {}, gr.aggregates, {
					$name: gr.name,
					$level: inverseLevel
				});
			...
			if (!groupings) this.groupings = null;
				...
				this.groupings.forEach(function(g) {
					...
					g.grouper = new Grouper(
						key,
						_objectSpread2({}, _this2.aggregates, {}, g.aggregates),
						function(r) {
